...
func main() {
	...
	// Trim
	a = []byte("  abcdabcdaabba  ")

	// 去除首尾指定字符串
	// abcdabcdaa
	fmt.Println("Trim")

	fmt.Println(string(bytes.Trim(a, "ab")))
	...
	fmt.Println(string(bytes.TrimLeft(a, "ab")))
	fmt.Println(string(bytes.TrimRight(a, "a\x00b")))

	fmt.Println(string(bytes.TrimSpace(a)))

	// TrimSuffix可以剔除一些不可打印的字符
	fmt.Println(string(bytes.TrimSuffix(a, []byte("aaa a\x00"))))
